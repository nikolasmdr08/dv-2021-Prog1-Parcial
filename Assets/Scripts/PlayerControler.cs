...
public class PlayerControler : MonoBehaviour
{
    ...
    void shoot()
    {
        ...
        if (Input.GetButton("Fire2") && !isShootingMisil)
        {
            if (stockBulletMisil > 0)
            {
                Instantiate(bulletMisil, misilLaucher.transform.position, misilLaucher.transform.rotation);
                isShootingMisil = true;
                stockBulletMisil--;
                Invoke("changeStateMisil", 0.25f);
            }
            else
            {
    ...
    void changeStateMisil()
    {
        isShootingMisil = false;
    }
